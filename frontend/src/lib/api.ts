...
export const AuthAPI = {
  ...
  verifyEmail: async (token: string): Promise<ApiResponse<{ message: string; user?: UserProfile }>> => {
    return apiFetch<{ message: string; user?: UserProfile }>('/api/auth/verify-email', {
      method: 'POST',
  ...
  verifyResetToken: async (token: string): Promise<ApiResponse<{ valid: boolean; email?: string }>> => {
    return apiFetch<{ valid: boolean; email?: string }>(`/api/auth/verify-reset-token/${token}`, {
      method: 'GET'
    });
  },
