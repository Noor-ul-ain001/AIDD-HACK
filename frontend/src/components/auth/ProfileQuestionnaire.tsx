...
export default function ProfileQuestionnaire({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    console.log('[PROFILE QUESTIONNAIRE] Form submitted with data:', profileData);
    setIsSubmitting(true);

    try {
      // Simulate API call
      console.log('[PROFILE QUESTIONNAIRE] Processing profile data...');
      await new Promise(resolve => setTimeout(resolve, 1000));
      console.log('[PROFILE QUESTIONNAIRE] Calling parent onSubmit callback...');
      onSubmit?.(profileData);
      console.log('[PROFILE QUESTIONNAIRE] Parent callback completed');
    } catch (error) {
      console.error('[PROFILE QUESTIONNAIRE] Error submitting profile:', error);
    } finally {
  ...
  const nextStep = () => {
    console.log('[PROFILE QUESTIONNAIRE] Next button clicked, current step:', currentStep);
    if (currentStep < 2) {
      setCurrentStep(currentStep + 1);
      console.log('[PROFILE QUESTIONNAIRE] Advanced to step:', currentStep + 1);
    }
  };

  const prevStep = () => {
    console.log('[PROFILE QUESTIONNAIRE] Previous button clicked, current step:', currentStep);
    if (currentStep > 0) {
      setCurrentStep(currentStep - 1);
      console.log('[PROFILE QUESTIONNAIRE] Went back to step:', currentStep - 1);
    }
  };
